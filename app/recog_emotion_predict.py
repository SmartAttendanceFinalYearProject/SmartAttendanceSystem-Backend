...
import insightface
from .recognizer import recognize_student   # Use reliable DB recognition
...
def predict_recog_emotion(image: Image.Image):
    """Recognition (DB) + Emotion (Model)"""
    try:
        img_cv = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
        faces = face_app.get(img_cv)

        if not faces:
            return {"student_id": "Unknown", "full_name": "Not registered as student", 
                    "emotion": "neutral", "recognized": False}
        ...
        # === Recognition using reliable DB method ===
