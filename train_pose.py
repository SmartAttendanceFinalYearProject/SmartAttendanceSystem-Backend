...
class PoseDataset(Dataset):
    def __init__(self, transform=None):
        ...
        self.image_paths = []
        self.labels = []
        self.class_to_idx = {cls: idx for idx, cls in enumerate(POSE_CLASSES)}

        print("Loading pose dataset - ONLY clear sitting & standing...")
            ...
            if pose_dir.exists():
                for img_path in pose_dir.glob("*.jpg"):
                    ...
                    if "sitting" in filename:
                        ...
                        continue  # Skip looking_up, looking_down, etc.

                    self.image_paths.append(img_path)
                    self.labels.append(label)

        print(f"Loaded {len(self.image_paths)} images | Classes: {POSE_CLASSES}")
    ...
    def __getitem__(self, idx):
        ...
        h, w = image.shape[:2]
        body_crop = image[0:int(h * 0.72), :, :]

        pil_image = Image.fromarray(body_crop)
...
if __name__ == "__main__":
    ...
    print(f"Starting Clean Sitting vs Standing Training from epoch {start_epoch + 1}...\n")
