...
model = RecogEmotionModel(num_students=num_students).to(device)
model.load_state_dict(checkpoint['model_state_dict'])
model.eval()

print("✅ Recognition + Emotion Model Loaded!")
...
def predict_recog_emotion(image: Image.Image):
    """Return student + emotion"""
    try:
        img_cv = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
        faces = face_app.get(img_cv)

        if not faces:
            return {"student_id": "Unknown", "full_name": "Not registered", "emotion": "neutral", "recognized": False}
        ...
        # Note: You need to map idx to real student name (you can improve this later)
        student_name = f"Student_{student_idx}"
